...
def collect_for_numeral(numeral):
    ...
    while True:
        ...
        if key == ord("c"):
            ...
            print(f"📸 Image {local_counter} captured for numeral {numeral}")
    ...
    print(f"✅ Completed numeral {numeral}: {local_counter} images collected")
    return True
...
def main():
    ...
    for numeral in range(start_numeral, 10):
        ...
        if numeral < 9 and continue_collection:
            print(f"\n⏳ Moving to next numeral: {numeral + 1}")
            time.sleep(2)
